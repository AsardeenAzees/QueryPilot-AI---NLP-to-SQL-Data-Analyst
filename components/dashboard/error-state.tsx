...
export function ErrorState({ title, message, errorCode, onRetry, canRetry = true }: { title: string; message: string; errorCode: string; onRetry: () => void; canRetry?: boolean }) {
  return (
    <div className="mt-5 flex flex-col gap-4 rounded-xl border border-red-200 bg-red-50 p-4 text-red-800 dark:border-red-900/80 dark:bg-red-950/45 dark:text-red-200 sm:flex-row sm:items-center sm:justify-between" role="alert" aria-live="assertive">
      <div className="flex items-start gap-3">
        ...
        <div><p className="text-sm font-bold">{title}</p><p className="mt-1 text-sm leading-6 opacity-90">{message}</p><p className="mt-2 font-mono text-xs opacity-75">Error code: {errorCode}</p></div>
      </div>
